...
if grep -q 'VERSION="10 (buster)"' /etc/os-release; then
 ...
 # apt-cache policy usbmount
  ...
  if [ $? = 0 ]; then
	...
	elif ! [ -e /home/pi/tmpu/usbmount_0.0.24_all.deb ] ; then
      wget -- no - check - certificate https://github.com/nicokaiser/usbmount/releases/download/0.0.24/usbmount_0.0.24_all.deb > /dev/null 2>&1
	fi
    if [ -e /home/pi/tmpu/usbmount_0.0.24_all.deb ] ; then
...
if grep -q 'VERSION="10 (buster)"' /etc/os-release; then
  ...
  if ! [ -e /tmp/$SKIN$Ver.zip ] ; then
    wget -P /tmp https://github.com/maltsevvv/repository-kodi/raw/master/kodi18/$SKIN/$SKIN$Ver.zip > /dev/null 2>&1
  elif ! [ -e /tmp/$SKIN$Ver.zip ] ; then
    wget -- no - check - certificate -P /tmp https://github.com/maltsevvv/repository-kodi/raw/master/kodi18/$SKIN/$SKIN$Ver.zip > /dev/null 2>&1
  fi
  ...
  echo "---------------------------------------------------------"
  if ! [ -e /tmp/$SKIN$Ver.zip ] ; then
    wget -P /tmp https://github.com/maltsevvv/repository-kodi/raw/master/kodi19/$SKIN/$SKIN$Ver.zip > /dev/null 2>&1
  elif ! [ -e /tmp/$SKIN$Ver.zip ] ; then
    wget -- no - check - certificate -P /tmp https://github.com/maltsevvv/repository-kodi/raw/master/kodi19/$SKIN/$SKIN$Ver.zip > /dev/null 2>&1
  fi
  ...
  if ! [ -e /tmp/$SKIN$Ver.zip ] ; then
    ...
    wget -- no - check - certificate -P /tmp https://github.com/maltsevvv/repository-kodi/raw/master/kodi20/$SKIN/$SKIN$Ver.zip > /dev/null 2>&1
  fi
elif ! [ -e /tmp/$SKIN$Ver.zip ] ; then
...
if [ -e /tmp/$SKIN$Ver.zip ] ; then
  ...
  if ! [ -e /tmp/$REPOSITORY.zip ] ; then
    ...
    wget -- no - check - certificate -P /tmp https://github.com/maltsevvv/repository-kodi/raw/master/$REPOSITORY.zip > /dev/null 2>&1
  fi
else
...
if [ -d $KODI$REPOSITORY ] ; then
  ...
  echo "---------------------------------------------------------"
  systemctl stop kodi && rm -r $KODI$REPOSITORY
fi
#Install Skin
if [ -e /tmp/$SKIN$Ver.zip ] ; then
  echo "---------------------------------------------------------"
