...
rpi_vga() {
  if grep -q 'VERSION="12 (bookworm)"' /etc/os-release; then
    ...
    if grep -q 'Raspberry Pi 3\|Raspberry Pi 4\|Raspberry Pi 5' $ModelPI; then
...
echo 'cp /boot/firmware/cmdline.txt /boot/firmware/cmdline.txt.rns'
echo 'cp /boot/firmware/cmdline.txt.backup /boot/firmware/cmdline.txt'
echo 'sudo reboot'
echo # Chek tv connect, entry pin, After
echo 'cp /boot/firmware/cmdline.txt /boot/firmware/cmdline.txt.backup'
echo 'cp /boot/firmware/cmdline.txt.rns /boot/firmware/cmdline.txt'
echo 'sudo reboot'
echo '---------------------------------------------------------'
