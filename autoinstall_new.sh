...
##############################################
if grep -q 'VERSION="10 (buster)"' /etc/os-release; then
  wget -P /tmp https://github.com/maltsevvv/maltsev-Kodi-Repo/raw/master/kodi18/skin.rnsd/skin.rnsd-18.3.0.zip
  ...
  wget -P /tmp https://github.com/maltsevvv/maltsev-Kodi-Repo/raw/master/kodi19/skin.rnsd/skin.rnsd-19.3.0.zip
  ...
  wget -P /tmp https://github.com/maltsevvv/maltsev-Kodi-Repo/raw/master/kodi20/skin.rnsd/skin.rnsd-20.3.0.zip
  wget -P /tmp https://github.com/maltsevvv/maltsev-Kodi-Repo/raw/master/repository.maltsev_kodi20/repository.maltsev_kodi20-1.0.0.zip
  sed -i -e '$i \  <addon>repository.maltsev_kodi19</addon>' /usr/share/kodi/system/addon-manifest.xml
fi
...
##############################################
#            EDIT /boot/config.txt           # /boot/firmware/config.txt
##############################################
echo "---------------------------------------------------------"
...
EOF
    if grep -q 'Raspberry Pi 4' /proc/device-tree/model; then
      sed -i 's/hdmi_force_hotplug/hdmi_force_hotplug:0/' /boot/config.txt
      sed -i 's/hdmi_ignore_edid/hdmi_ignore_edid:0/' /boot/config.txt
      sed -i 's/hdmi_group/hdmi_group:0/' /boot/config.txt
      sed -i 's/hdmi_mode/hdmi_mode:0/' /boot/config.txt
...
EOF
    ...
    sed -i 's/dtoverlay=vc4-kms-v3d/dtoverlay=vc4-fkms-v3d/' /boot/firmware/config.txt
...
EOF
    if grep -q 'Raspberry Pi 4' /proc/device-tree/model; then
      sed -i 's/hdmi_force_hotplug/hdmi_force_hotplug:0/' /boot/firmware/config.txt
      sed -i 's/hdmi_ignore_edid/hdmi_ignore_edid:0/' /boot/firmware/config.txt
      sed -i 's/hdmi_group/hdmi_group:0/' /boot/firmware/config.txt
      sed -i 's/hdmi_mode/hdmi_mode:0/' /boot/firmware/config.txt
